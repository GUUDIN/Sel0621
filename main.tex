...
Para determinar sua faixa de operação adequada, devemos estabelecer os limites de tolerância admissíveis. Considerando:

\begin{itemize}
    \item Corrente de referência: $I_0 = 25\,\mu\text{A}$ (valor nominal para $V_{DD} = 3{,}0$\,V)
    \item Tolerância especificada: $\pm 2\%$
    \item Limite inferior: $I_0(0{,}98) = 25 \times 0{,}98 = 24{,}5\,\mu\text{A}$
    \item Limite superior: $I_0(1{,}02) = 25 \times 1{,}02 = 25{,}5\,\mu\text{A}$
\end{itemize}

Portanto, a faixa aceitável de corrente é: $24{,}5\,\mu\text{A} < I_S < 25{,}5\,\mu\text{A}$.

Através da simulação de $I_S$ em função de $V_{DD}$, observamos o comportamento da fonte de corrente apresentado na Figura~\ref{fig:corrente_maxima_minima}. Da análise do gráfico, identificamos os pontos críticos:

\begin{itemize}
    \item Para $I_S = 24{,}5\,\mu\text{A}$ (limite inferior): $V_{DD} = 2{,}78$\,V
    \item Para $I_S = 25{,}5\,\mu\text{A}$ (limite superior): $V_{DD} = 3{,}18$\,V
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/corrente_maxima_minima.png}
    \caption{Corrente de saída $I_S$ em função de $V_{DD}$, destacando os limites de tolerância de $\pm 2\%$. A faixa operacional aceitável está entre $V_{DD} = 2{,}78$\,V e $V_{DD} = 3{,}18$\,V.}
...
Conforme observado na Questão 9, a corrente $I_S$ varia significativamente com $V_{DD}$ devido ao efeito de \textbf{modulação de canal}, descrito por:
\begin{equation}
I_D = I_{D0}\left(1 + \lambda V_{DS}\right)
\end{equation}

Para reduzir esta sensibilidade, o objetivo é diminuir $\lambda$ e aumentar a impedância de saída $r_{out} = 1/(\lambda I_D)$. Duas estratégias principais podem ser adotadas:

\subsubsection*{1. Aumento do Comprimento de Canal (L)}

Como $\lambda$ é inversamente proporcional a $L$, aumentar o comprimento dos transistores reduz diretamente o efeito da modulação de canal. Quando $V_{DS}$ aumenta, o estreitamento efetivo do canal ($\Delta L$) torna-se percentualmente menor em transistores com maior $L$ nominal, resultando em corrente mais estável.

Neste projeto, é mais vantajoso aumentar o $L$ dos \textbf{transistores PMOS}, que já possuem dimensões compactas. Os transistores NMOS, com $W$ já bastante elevado, se tornarem excessivamente grandes caso seus comprimentos sejam aumentados proporcionalmente, comprometendo a área total do circuito.

\subsubsection*{2. Mudança de Topologia do Espelho}

Substituir o espelho simples por topologias de alta impedância aumenta drasticamente $r_{out}$:

\begin{itemize}
    \item \textbf{Espelho Cascode:} conexão em série de transistores eleva a impedância para $g_m r_o^2$;
    \item \textbf{Espelho de Wilson:} utiliza realimentação local para compensar variações de tensão, oferecendo impedância ainda maior.
\end{itemize}

A versão \textbf{simétrica do Wilson} é preferível por manter a simetria do circuito e melhorar o casamento entre componentes.

\subsubsection*{Proposta de Implementação}

Para este projeto, recomenda-se combinar ambas as abordagens:

\begin{enumerate}
    \item Aumentar moderadamente o $L$ dos PMOS (fator de 1,5× a 2×) para reduzir $\lambda$;
    \item Implementar \textbf{espelho de Wilson simétrico} no ramo NMOS, evitando aumento excessivo de dimensões.
\end{enumerate}

Esta solução equilibra desempenho e área, proporcionando excelente regulação de corrente sem necessidade de circuitos adicionais complexos, e ampliando significativamente a faixa operacional de $V_{DD}$.
...
Segundo o manual ENG-183 Rev.3.0, o desenho de resistores em tecnologia CMOS 0,35\,$\mu$m utiliza camadas específicas de layout:

\begin{itemize}
    \item \textbf{Camada RESDEF:} Define a região que o resistor irá ocupar no layout
    \item \textbf{Camada RESTRM:} Delimita os terminais do resistor, removendo porções da área definida por RESDEF
    \item \textbf{Geometria em serpentina:} Para valores elevados de resistência, emprega-se formato sinuoso para otimizar o uso da área de silício
\end{itemize}

O material mais adequado é o \textbf{Polisilício de Alta Resistividade (RPOLYH)}, que oferece $R_{\square} = 1200\,\Omega/\square$. Esta escolha se justifica por permitir obter valores elevados de resistência com área mínima de silício.

Para o resistor de aproximadamente 1\,k$\Omega$ necessário neste projeto, o RPOLYH é a escolha ótima, minimizando a área ocupada através de layout sinuoso e mantendo boa precisão.
